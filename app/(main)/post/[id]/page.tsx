...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full max-w-2xl border-x border-white/[0.04] bg-[#111111] min-h-screen pb-20">
      {/* Navigation Header for Desktop */}
      <div className="hidden md:flex sticky top-0 z-30 items-center gap-3 border-b border-white/[0.06] bg-[#111111]/80 px-4 py-3 backdrop-blur-2xl">
        <Link href="/" aria-label="Back to feed">
          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full hover:bg-white/10">
            <ArrowLeft className="h-4.5 w-4.5" />
          </Button>
        </Link>
        <h1 className="text-lg font-bold font-heading tracking-tight">Post</h1>
      </div>
